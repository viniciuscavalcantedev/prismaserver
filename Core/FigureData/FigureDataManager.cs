...
using log4net;
using Bios.Core.FigureData.Types;
using System.Xml;
using Bios.HabboHotel.Catalog.Clothing;
using Bios.HabboHotel.Users.Clothing.Parts;
using Bios.Core.FigureDataManager;

namespace Bios.Core.FigureData
{
    public class FigureDataManager
    {
        private static readonly ILog log = LogManager.GetLogger("Bios.Core.FigureData");
        ...
        public string ProcessFigure(string figure, string gender, ICollection<ClothingParts> clothingParts, bool hasHabboClub)
        {
            ...
            {
                ICollection<ClothingItem> purchasableParts = BiosEmuThiago.GetGame().GetCatalog().GetClothingManager().GetClothingAllParts;

...
namespace Bios.Core
{
    class CatalogSettings
    {
        ...
        private static readonly ILog log = LogManager.GetLogger("Bios.Core");

        public static bool RunCatalogSettings()
        {
            foreach (var @params in from line in File.ReadAllLines("BiosConfingThiago/CatalogBiosEmuThiago.ini", Encoding.Default) where !String.IsNullOrWhiteSpace(line) && line.Contains("=") select line.Split('='))
            {
                switch (@params[0])
                {
            ...
            log.Info("» Catalogo Configurações -> PRONTO - BY: Thiago Araujo!");
            return true;
        }
    }
}
...
using Bios.Communication.Packets.Outgoing.Rooms.Freeze;
using Bios.HabboHotel.Rooms.Games.Teams;
using Bios.HabboHotel.Items.Wired;
using Bios.Communication.Packets.Outgoing.Rooms.Notifications;

namespace Bios.HabboHotel.Rooms.Games.Freeze
{
    public class Freeze
    {
        private Room _room;
        ...
        {
            return ExitTeleports.Values.ToList()[BiosEmuThiago.GetRandomNumber(0, ExitTeleports.Count - 1)];
        }
        ...
        private void CountTeamPoints()
        {
            ...
            foreach (RoomUser User in this._room.GetRoomUserManager().GetUserList().ToList())
            {
                ...
                BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FreezePlayer", 1);
            }
        }
                ...
                case FreezePowerUp.HEART:
                    {
                        if (User.FreezeLives < 5)
                        {
                            ...
                            BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FreezeWinner", 1);
                        }

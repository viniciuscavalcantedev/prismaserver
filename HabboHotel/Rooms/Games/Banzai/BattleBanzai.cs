...
using Bios.Utilities.Enclosure;
using System.Linq;
...
namespace Bios.HabboHotel.Rooms.Games.Banzai
{
    public class BattleBanzai
    {
        ...
        public void BanzaiStart()
        {
            ...
            timestarted = BiosEmuThiago.GetUnixTimestamp();
        ...
        public void BanzaiEnd(bool userTriggered = false)
        {
            ...
            if (winners != TEAM.NONE)
            {
                ...
                foreach (RoomUser User in Winners.ToList())
                {
                    ...
                    {
                        if (BiosEmuThiago.GetUnixTimestamp() - timestarted > 5)
                        {
                            BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallTilesLocked", User.LockedTilesCount);
                            BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallPlayer", 1);
                        }
                    }
                    if (winners == TEAM.BLUE)
                    {
                        ...
                        {
                            if (BiosEmuThiago.GetUnixTimestamp() - timestarted > 5)
                                BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallWinner", 1);

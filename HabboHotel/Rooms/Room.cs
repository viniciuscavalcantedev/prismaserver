...
using Bios.HabboHotel.Rooms.Instance;

using Bios.HabboHotel.Items.Data.Toner;
using Bios.HabboHotel.Rooms.Games.Freeze;
...
using Bios.HabboHotel.Rooms.Games.Banzai;
...
using Bios.HabboHotel.Rooms.Games.Football;
    ...
    public class Room : RoomData
    {
        ...
        public void AddBan(int pId, long Time)
        {
            if (!Bans.ContainsKey(Convert.ToInt32(pId)))
                Bans.Add(pId, BiosEmuThiago.GetUnixTimestamp() + Time);

            using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.RunQuery("REPLACE INTO `room_bans` VALUES (" + pId + ", " + Id + ", " + (BiosEmuThiago.GetUnixTimestamp() + Time) + ")");
            }
        }
        ...
        public bool HasBanExpired(int pId)
        {
            ...
            if (Bans[pId] < BiosEmuThiago.GetUnixTimestamp())
                return true;
        ...
        public void LoadPromotions()
        {
            ...
            using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                ...
                {
                    if (Convert.ToDouble(GetPromotion["timestamp_expire"]) > BiosEmuThiago.GetUnixTimestamp())
        ...
        public void AssignNewOwner(Room ForSale, RoomUser Buyer, RoomUser Seller)
        {
                ...
                if (ForSale.Group != null)
                {
                    ...
                    if (GroupID > 0)
                    {
                        ...
                        foreach (int UserID in ForSale.Group.GetAllMembers)
                        {
                            ...
                            GameClient UserClient = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(UserID);
                            if (UserClient == null)
                                continue;
                    ...
                    BiosEmuThiago.GetGame().GetGroupManager().DeleteGroup(ForSale.Group.Id);
            ...
            BiosEmuThiago.GetGame().GetRoomManager().UnloadRoom(Room, true);
        ...
        public void OnUserShoot(GameClient client, Item Ball)
        {
            ...
            foreach (Item item in this.GetRoomItemHandler().GetFurniObjects(Ball.GetX, Ball.GetY).ToList())
            {
                if (item.GetBaseItem().ItemName.StartsWith("fball_goal_"))
                {
                    ...
                    BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FootballGoalScored", 1);
        ...
        public void ProcessRoom()
        {
            ...
            try
            {
                ...
                {
                    BiosEmuThiago.GetGame().GetRoomManager().UnloadRoom(this);
                    return;
                }
        ...
        public bool CheckMute(GameClient Session)
        {
            ...
            {
                if (MutedUsers[Session.GetHabbo().Id] < BiosEmuThiago.GetUnixTimestamp())
                {
        ...
        public void AddChatlog(int Id, string Message)
        {
            using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                ...
                dbClient.AddParameter("time", BiosEmuThiago.GetUnixTimestamp());
                dbClient.RunQuery();
            }
        }

...
namespace Bios.HabboHotel.Rooms.Chat.Commands.User
{
    class roomAccepttOffer : IChatCommand
    {
        ...
        public void NewRoomOwner(Room RoomForSale, RoomUser BoughtRoomUser, RoomUser SoldRoomUser)
        {
                ...
                if (RoomForSale.Group != null)
                {
                    ...
                    if (GroupId > 0)
                    {
                        ...
                        foreach (int MemberId in RoomForSale.Group.GetAllMembers)
                        {
                            ...
                            GameClients.GameClient MemberClient = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(MemberId);
                    ...
                    BiosEmuThiago.GetGame().GetGroupManager().DeleteGroup(RoomForSale.Group.Id);
            ...
            //Unload the Room
            Room R = null;
            if (!BiosEmuThiago.GetGame().GetRoomManager().TryGetRoom(RoomForSale.Id, out R))
                return;
            List<RoomUser> UsersToReturn = RoomForSale.GetRoomUserManager().GetRoomUsers().ToList();
            BiosEmuThiago.GetGame().GetNavigator().Init();
            BiosEmuThiago.GetGame().GetRoomManager().UnloadRoom(R, true);

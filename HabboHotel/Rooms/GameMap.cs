...
namespace Bios.HabboHotel.Rooms
{
	public class Gamemap
	{
		...
		public Gamemap(Room room)
		{
			...
			mStaticModel = BiosEmuThiago.GetGame().GetRoomManager().GetModel(room.ModelName);
			if (mStaticModel == null)
			{
				BiosEmuThiago.GetGame().GetRoomManager().LoadModel(room.ModelName);
				mStaticModel = BiosEmuThiago.GetGame().GetRoomManager().GetModel(room.ModelName);
			}
		...
		public Point GetRandomWalkableSquare()
		{
			...
			int RandomNumber = BiosEmuThiago.GetRandomNumber(0, walkableSquares.Count);
			int i = 0;
				...
				if (mItemHeightMap[Coord.X, Coord.Y] <= Item.TotalHeight)
				{
					...
					switch (Item.GetBaseItem().InteractionType)
					{
						...
						case InteractionType.SILLAGUIA:
							...
							break;
                        case InteractionType.ICES_SNOM:
                            mUserItemEffect[Coord.X, Coord.Y] = 8;
                            break;
                    }
		...
		public bool IsValidStep2(RoomUser User, Vector2D From, Vector2D To, bool EndOfPath, bool Override)
		{
			...
			if (Items.Count > 0)
			{
				...
				if (HasGroupGate)
				{
					...
					{
						Group Group = null;
						if (!BiosEmuThiago.GetGame().GetGroupManager().TryGetGroup(I.GroupId, out Group))
							return false;

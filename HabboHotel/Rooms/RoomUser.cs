...
namespace Bios.HabboHotel.Rooms
{
    ...
    {
        public bool RoomOfferPending = false;
        public string RoomOffer = "";
        public int RoomOfferUser = 0;
        ...
        public string GetUsername()
        {
            ...
            if (GetClient() != null)
            {
                if (GetClient().GetHabbo() != null)
                {
                    ...
                    return BiosEmuThiago.GetUsernameById(HabboId);
        ...
        public bool IncrementAndCheckFlood(out int MuteTime)
        {
            ...
            else if (ChatSpamCount >= 6)
            {
                ...
                GetClient().GetHabbo().FloodTime = BiosEmuThiago.GetUnixTimestamp() + MuteTime;
        ...
        public void OnChat(int Colour, string Message, bool Shout)
        {
            ...
            if (Shout)
                Packet = new ShoutComposer(VirtualId, ColouredMessage, BiosEmuThiago.GetGame().GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
            else
                Packet = new ChatComposer(VirtualId, ColouredMessage, BiosEmuThiago.GetGame().GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
        ...
        public void SendNameColourPacket()
        {
            ...
            else
            {
                if (GetClient().GetHabbo()._NamePrefixColor == "rainbow")
                    Username = "<font color='" + BiosEmuThiago.RainbowT() + "'>[" + GetClient().GetHabbo()._NamePrefix + "]</font> <font color='" + GetClient().GetHabbo().chatHTMLColour + "'>" + GetClient().GetHabbo().Username + "</font>";
                else
        ...
        public GameClient GetClient()
        {
            ...
            if (mClient == null)
                mClient = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(HabboId);
            return mClient;
        }
        ...
        {
            if (mRoom == null)
                if (BiosEmuThiago.GetGame().GetRoomManager().TryGetRoom(RoomId, out mRoom))
                    return mRoom;
    ...
    public static class ByteToItemEffectEnum
    {
        public static ItemEffectType Parse(byte pByte)
        {
            switch (pByte)
            {
                ...
                case 10:
                    return ItemEffectType.IcesSnom;
                default:
                    return ItemEffectType.NONE;
            }
        }

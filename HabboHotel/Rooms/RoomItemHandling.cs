...
namespace Bios.HabboHotel.Rooms
{
    public class RoomItemHandling
    {
        ...
        public void UpdateWiredScoreBord()
        {
            ...
            {
                if (scoreitem.GetBaseItem().InteractionType == InteractionType.wired_score_board)
                {
            ...
            foreach (Item Item in Items.ToList())
            {
                ...
                if (Item.IsFloorItem)
                {
                    if (!_room.GetGameMap().ValidTile(Item.GetX, Item.GetY))
                    {
                        ...
                        GameClient Client = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(Item.UserID);
                        if (Client != null)
                        {
        ...
        public bool SetFloorItem(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false)
        {
            ...
            if (newItem)
            {
                ...
                {
                    if (Session != null)
                        Session.SendNotification(BiosEmuThiago.GetGame().GetLanguageManager().TryGetValue("room.item.already_placed"));
                    _room.GetGameMap().RemoveFromMap(Item);
                    return true;
                }

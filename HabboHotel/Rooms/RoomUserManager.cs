...
namespace Bios.HabboHotel.Rooms
{
    public class RoomUserManager
    {
        ...
        public bool AddAvatarToRoom(GameClient Session)
        {
            ...
            // Dar Efeito embaixador By: Thiago Araujo
            if (Session.GetHabbo().isLoggedIn && Session.GetHabbo().Rank == ExtraSettings.AmbassadorMinRank)
            {
                Session.GetHabbo().Effects().ApplyEffect(178);
            }
            // Dar Efeito staff no login staff By: Thiago Araujo
            ...
            if (_room.Id == BiosEmuThiago.Quartovip)
            {
                DataRow vipsthiago = null;
                using (var dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.SetQuery("SELECT Vipss FROM users WHERE id = '" + Session.GetHabbo().Id + "'");
                    vipsthiago = dbClient.getRow();
                }

                if (Convert.ToBoolean(vipsthiago["Vipss"]) == false)
                {

                    if (Session.GetHabbo().Rank == 1)
                    {
                        Session.SendMessage(new RoomNotificationComposer("energy_low_warning", 3, "Você " + Session.GetHabbo().Username + " não pode entra no quarto pra Vips é staff. Pois você não é um!", ""));
                        if (!Session.GetHabbo().InRoom)
                            Session.SendMessage(new RoomForwardComposer(0));
                        else
            ...
            if (_room.Id == BiosEmuThiago.Prisao)
            {
                DataRow usersthiago = null;
                using (var dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.SetQuery("SELECT Presidio FROM users WHERE id = '" + Session.GetHabbo().Id + "'");
                    usersthiago = dbClient.getRow();
                }

                if (Convert.ToBoolean(usersthiago["Presidio"]) == false)
                {

                    if (Session.GetHabbo().Rank == 1)
                    {
                        string thiago = Session.GetHabbo().Look;
                        Session.SendMessage(new RoomNotificationComposer("police_announcement", 3, "Você não pode entra na prisão so presos é staff!.", ""));
                        if (!Session.GetHabbo().InRoom)
                            Session.SendMessage(new RoomForwardComposer(0));
                        else
                            Session.GetHabbo().PrepareRoom(0, "");
            ...
            if (Session.GetHabbo().Rank > 0)
            {
                ...
                if (Convert.ToBoolean(presothiago["Presidio"]) == true)
                {
                    if (_room.Id != BiosEmuThiago.Prisao)
                    {
                        Session.SendMessage(new RoomNotificationComposer("police_announcement", 3, "Você " + Session.GetHabbo().Username + " esta preso e não pode entra nos outros quartos. So na prisão!", ""));
                        if (!Session.GetHabbo().InRoom)
                            Session.SendMessage(new RoomForwardComposer(BiosEmuThiago.Prisao));
                        else
                            Session.GetHabbo().PrepareRoom(BiosEmuThiago.Prisao, "");
                    }
                }
            }

            DataRow tutorialthiago = null;
            int tutorialid = 0;
            if (_room != null)
                tutorialid = _room.Id;
            using (var dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("SELECT tutorial FROM rooms WHERE id = '" + tutorialid + "'");
                tutorialthiago = dbClient.getRow();
            }

            if (Convert.ToBoolean(tutorialthiago["tutorial"]) == true)
            {
                ServerPacket notif = new ServerPacket(ServerPacketHeader.NuxAlertMessageComposer);
                notif.WriteString("habbopages/quartos/tutoriais/" + tutorialid + ".txt");
                Session.SendMessage(notif);
                Session.SendMessage(new RoomNotificationComposer("frank", "message", "Oi" + " " + Session.GetHabbo().Username + " " + ", essa sala possui uma notificação. Leia com atenção!"));
            }
        ...
        public void RemoveUserFromRoom(GameClient Session, Boolean NotifyClient, Boolean NotifyKick = false)
        {
            try
            {
                ...
                if (User != null)
                {
                    ...
                    {
                        dbClient.runFastQuery("UPDATE user_roomvisits SET exit_timestamp = '" + BiosEmuThiago.GetUnixTimestamp() + "' WHERE room_id = '" + _room.RoomId + "' AND user_id = '" + Session.GetHabbo().Id + "' ORDER BY exit_timestamp DESC LIMIT 1");
        ...
        private void RemoveRoomUser(RoomUser user)
        {
			...
			if (this._users.TryRemove(user.InternalRoomID, out RoomUser toRemove))
			{
        ...
        public void OnCycle()
        {
            ...
            try
            {
                ...
                foreach (RoomUser User in GetUserList().ToList())
                {
                    ...
                    //int GameLength = 50;
                    //GameLength = 50;
                    //if (GameLength == 40 && _room.UsersNow > 0)
                    //{
                    //    BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_RoomDecoHosting", 1);
                    //    User.GetClient().SendMessage(new RoomNotificationComposer("furni_placement_error", "message", "Testando 1"));
                    //}
                ...
                {
                    GameClient client = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(toRemove.HabboId);
                    if (client != null)
                    {
        ...
        public void UpdateUserStatus(RoomUser User, bool cyclegameitems)
        {
            ...
            try
            {
                ...
                if (BiosEmuThiago.GetUnixTimestamp() > BiosEmuThiago.GetUnixTimestamp() + User.SignTime)
                {
                    if (User.Statusses.ContainsKey("sign"))
                    {
                        ...
                        case InteractionType.ARROW:
                            {
                                if (User.GoalX == Item.GetX && User.GoalY == Item.GetY)
                                {
									...
									if (!BiosEmuThiago.GetGame().GetRoomManager().TryGetRoom(User.GetClient().GetHabbo().CurrentRoomId, out Room Room))
										return;
                                    ...
                                    else
                                    {
                                        ...
                                        if (TeleRoomId == Room.RoomId)
                                        {
                                            ...
                                            {
                                                if (User.GetClient() != null)
                                                    User.GetClient().SendWhisper("Ei, essa flecha é fraca!");
                                                return;
                                            }
                                            else
                                            {
                        ...
                        case InteractionType.PINATA:
                            {
                                if (User.IsWalking && Item.ExtraData.Length > 0)
                                {
                                    ...
                                    if (givenHits < 1 && User.CurrentEffect == 158)
                                    {
                                        ...
                                        if (givenHits == 1)
                                        {
                                            BiosEmuThiago.GetGame().GetPinataManager().ReceiveCrackableReward(User, _room, Item);
                                            BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataBreaker", 1);
                                        }

                                        #region Achievements
                                        BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataWhacker", 1);
                                        #endregion

                                    }
                                    else
                                    {
                                        User.GetClient().SendWhisper("Ops, você não esta com a vareta! digite o comando[:efeito 158] (fix By: Thiago Araujo)");
                                        return;
                                    }
                                }
                                break;
        ...
        private void UpdateUserEffect(RoomUser User, int x, int y)
        {
            ...
            try
            {
                ...
                if(User.GetClient().GetHabbo().CurrentRoom.GetRoomUserManager().GetRoomUserByHabbo(User.GetClient().GetHabbo().Id).CurrentEffect == 39 || User.GetClient().GetHabbo().CurrentRoom.GetRoomUserManager().GetRoomUserByHabbo(User.GetClient().GetHabbo().Id).CurrentEffect == 38)
                {
                    if(User.IsWalking)
                    {
                        BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_TagC", 1);
                    }

                }
                if (User.GetClient().GetHabbo().CurrentRoom.GetRoomUserManager().GetRoomUserByHabbo(User.GetClient().GetHabbo().Id).CurrentEffect == 55 || User.GetClient().GetHabbo().CurrentRoom.GetRoomUserManager().GetRoomUserByHabbo(User.GetClient().GetHabbo().Id).CurrentEffect == 56)
                {
                    if (User.IsWalking)
                    {
                        BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_RbTagA", 1);
                    }

                }
                if (User.GetClient().GetHabbo().CurrentRoom.GetRoomUserManager().GetRoomUserByHabbo(User.GetClient().GetHabbo().Id).CurrentEffect == 97)
                {
                    if (User.IsWalking)
                    {
                        BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_snowBoardBuild", 1);
                    }
                ...
                if (NewCurrentUserItemEffect > 0)
                    ...
                    Room Room = User.GetClient().GetHabbo().CurrentRoom;
                    RoomUser Actor = Room.GetRoomUserManager().GetRoomUserByHabbo(User.GetClient().GetHabbo().Id);
                    if (Type != User.CurrentItemEffect)
                    {
                            switch (Type)
                            {
                                ...
                                case ItemEffectType.IcesSnom:
                                    {
                                        User.GetClient().GetHabbo().Effects().ApplyEffect(97);
                                ...
                                case ItemEffectType.Normalskates:
                                    {
                                        User.GetClient().GetHabbo().Effects().ApplyEffect(User.GetClient().GetHabbo().Gender == "M" ? 56 : 55);
                                        User.CurrentItemEffect = Type;
                                        break;
                                    }

...
namespace Bios.HabboHotel.Users.Messenger
{
    public class HabboMessenger
    {
        ...
        public void ProcessOfflineMessages()
        {
            ...
            using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetMessages != null)
                {
                    GameClient Client = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(this._userId);
                    if (Client == null)
                        return;
        ...
        public void Destroy()
        {
            IEnumerable<GameClient> onlineUsers = BiosEmuThiago.GetGame().GetClientManager().GetClientsById(_friends.Keys);
        ...
        public void CreateFriendship(int friendID)
        {
            ...
            GameClient User = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (User != null)
                BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(User, "ACH_FriendListSize", 1);

            GameClient thisUser = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(_userId);
            if (thisUser != null)
                BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(thisUser, "ACH_FriendListSize", 1);
        }
        ...
        {
            GameClient friend = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (friend == null || friend.GetHabbo() == null)
            {
                ...
                newFriend = new MessengerBuddy(friendID, Convert.ToString(dRow["username"]), Convert.ToString(dRow["look"]), Convert.ToString(dRow["motto"]), Convert.ToInt32(dRow["last_online"]),
                    BiosEmuThiago.EnumToBool(dRow["hide_online"].ToString()), BiosEmuThiago.EnumToBool(dRow["hide_inroom"].ToString()));
            }
            else
            {
        ...
        public bool RequestBuddy(string UserQuery)
        {
            ...
            GameClient client = BiosEmuThiago.GetGame().GetClientManager().GetClientByUsername(UserQuery);
            if (client == null)
            {
                DataRow Row = null;
                using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
                ...
                hasFQDisabled = BiosEmuThiago.EnumToBool(Row["block_newfriends"].ToString());
            }
            else
            {
            ...
            BiosEmuThiago.GetGame().GetQuestManager().ProgressUserQuest(GetClient(), QuestType.ADD_FRIENDS);

            GameClient ToUser = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (ToUser == null || ToUser.GetHabbo() == null)
                return true;

            MessengerRequest Request = new MessengerRequest(ToId, _userId, BiosEmuThiago.GetGame().GetClientManager().GetNameById(_userId));

            ToUser.GetHabbo().GetMessenger().OnNewRequest(_userId);

            UserCache ThisUser = BiosEmuThiago.GetGame().GetCacheManager().GenerateUser(_userId);
        ...
        public void OnNewRequest(int friendID)
        {
            if (!_requests.ContainsKey(friendID))
                _requests.Add(friendID, new MessengerRequest(_userId, friendID, BiosEmuThiago.GetGame().GetClientManager().GetNameById(friendID)));
        }
        ...
        public void SendInstantMessage(int ToId, string Message)
        {
            ...
            #region Custom Chats
            var Group = BiosEmuThiago.GetGame().GetGroupManager().GetGroupsForUser(GetClient().GetHabbo().Id).Where(c => c.HasChat).ToList();
            foreach (var gp in Group)
            {
                if (ToId == int.MinValue + gp.Id) // int.MaxValue
                {
                    //BiosEmuThiago.GetGame().GetClientManager().SendMessaget(new FuckingConsoleMessageComposer(ToId, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), GetClient().GetHabbo().Id);
                    BiosEmuThiago.GetGame().GetClientManager().GroupChatAlert(new FuckingConsoleMessageComposer(int.MinValue + gp.Id, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), gp, GetClient().GetHabbo().Id);
                    return;
                }
            ...
            {
                BiosEmuThiago.GetGame().GetClientManager().StaffAlert(new FuckingConsoleMessageComposer(ToId, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), GetClient().GetHabbo().Id);
                return;
            }
            else if (GetClient().GetHabbo().Rank == 2 || GetClient().GetHabbo().Rank >= 12 && ToId == (int.MinValue + 1))
            {
                BiosEmuThiago.GetGame().GetClientManager().GuideAlert(new FuckingConsoleMessageComposer(ToId, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), GetClient().GetHabbo().Id);
                return;
            }
            #endregion
            ...
            {
                GetClient().GetHabbo().MessengerSpamTime = BiosEmuThiago.GetUnixTimestamp() + 60;
            ...
            else if (GetClient().GetHabbo().MessengerSpamTime > BiosEmuThiago.GetUnixTimestamp())
            {
                double Time = GetClient().GetHabbo().MessengerSpamTime - BiosEmuThiago.GetUnixTimestamp();
            ...
            GameClient Client = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (Client == null || Client.GetHabbo() == null || Client.GetHabbo().GetMessenger() == null)
            {
                using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
                {
        ...
        {
            IEnumerable<GameClient> MyFriends = BiosEmuThiago.GetGame().GetClientManager().GetClientsById(this._friends.Keys);
        ...
        {
            return BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(this._userId);
        }

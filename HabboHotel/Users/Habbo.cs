...
using Bios.HabboHotel.Users.Permissions;
using Bios.HabboHotel.Subscriptions;
using Bios.HabboHotel.Users.Ignores;
using Bios.HabboHotel.Users.Polls;
using Bios.Communication.Packets.Outgoing.Rooms.Notifications;
using Bios.Communication.Packets.Outgoing.LandingView;

namespace Bios.HabboHotel.Users
{
    public class Habbo
    {
        private static readonly ILog log = LogManager.GetLogger("Bios.HabboHotel.Users");
        ...
        public Habbo(int Id, string Username, int Rank, string Motto, string Look, string Gender, int Credits, int ActivityPoints, int HomeRoom,
            ...
            int GOTWPoints, int UserPoints, bool IgnoreInvites, double TimeMuted, double TradingLock, bool AllowGifts, int FriendBarState, bool DisableForcedEffects, bool AllowMimic, bool AllowEvents, int VIPRank, string chatHTMLcolour, string Chatcolour, string NamePrefix, string NamePrefixColor, int BubbleId, byte publicistalevel, byte guidelevel, byte builder, byte croupier, string citizenShip, byte TargetedBuy, int BonusPoints)
        {
            _id = Id;
            _username = Username;
            _rank = Rank;
            ...
            _footballLook = BiosEmuThiago.FilterFigure(Look.ToLower());
            ...
            if (_tradingLockExpiry > 0 && BiosEmuThiago.GetUnixTimestamp() > TradingLockExpiry)
            {
            ...
            _sessionStart = BiosEmuThiago.GetUnixTimestamp();
            _messengerSpamCount = 0;
            _messengerSpamTime = 0;
            _creditsTickUpdate = Convert.ToInt32(BiosEmuThiago.GetGame().GetSettingsManager().TryGetValue("user.currency_scheduler.tick"));
            int isEntra;
            int isMedal;
                ...
                try
                {
                    ...
                    if (Convert.ToString(StatRow["respectsTimestamp"]) != DateTime.Today.ToString("MM/dd"))
                    {
                        ...
                        if (_permissions.HasRight("mod_tool"))
                            DailyRespects = 20;
                        else if (BiosEmuThiago.GetGame().GetSubscriptionManager().TryGetSubscriptionData(VIPRank, out SubData))
            ...
            Group G = null;
            if (!BiosEmuThiago.GetGame().GetGroupManager().TryGetGroup(_habboStats.FavouriteGroupId, out G))
        ...
        public Room CurrentRoom
        {
            get
            {
				...
				if (BiosEmuThiago.GetGame().GetRoomManager().TryGetRoom(CurrentRoomId, out Room _room))
					return _room;
        ...
        {
            int level = _Talents.Values.Select(current => BiosEmuThiago.GetGame().GetTalentManager().GetTalent(current.TalentId).Level).Concat(new[] { 1 }).Max();
            return level;
        }
       ...
       public void Init(GameClient client, UserData.UserData data)
        ...
        public void OnDisconnect()
        {
            ...
            BiosEmuThiago.GetGame().GetClientManager().UnregisterClient(Id, Username);
        ...
        public void CheckBonusTimer()
        {
            try
            {
                ...
                if (_bonusTickUpdate <= 0)
                {
                    ...
                    this._client.GetHabbo().BonusPoints += 1;
                    this._client.SendMessage(new HabboActivityPointNotificationComposer(this._BonusPoints, BonusUpdate, 101));
                    this._client.SendMessage(new RoomAlertComposer("Parabéns! Você recebeu um ponto bônus por estar logado por 2 horas."));
                    this._client.SendMessage(new BonusRareMessageComposer(this._client));
                    this.BonusUpdateTick = Convert.ToInt32(BiosEmuThiago.GetGame().GetSettingsManager().TryGetValue("user.bonusr_scheduler.tick"));
                }
            }
            catch { }
        }
        ...
        public void CheckCreditsTimer()
        {
            try
            {
                ...
                if (_creditsTickUpdate <= 0)
                {
                    ...
                    int CreditUpdate = Convert.ToInt32(BiosEmuThiago.GetConfig().data["Moedas"]);
                    int DucketUpdate = Convert.ToInt32(BiosEmuThiago.GetConfig().data["Duckets"]);
                    int DiamondUpdate = Convert.ToInt32(BiosEmuThiago.GetConfig().data["Diamantes"]);
                    bool flag2 = Convert.ToBoolean(BiosEmuThiago.GetConfig().data["MensagemAoReceber"]);

                    if (flag2)
                    {
                    if (BiosEmuThiago.GetGame().GetSubscriptionManager().TryGetSubscriptionData(_vipRank, out SubscriptionData SubData))
					{
                    ...
                    GetClient().SendMessage(RoomNotificationComposer.SendBubble("command_notification_credits", "Você recebeu " + CreditUpdate + " créditos, " + DucketUpdate + " duckets, " + DiamondUpdate + " Diamantes, " +  "por estar conectado 40 minutos.", ""));

                    CreditsUpdateTick = Convert.ToInt32(BiosEmuThiago.GetGame().GetSettingsManager().TryGetValue("user.currency_scheduler.tick"));
                }
                }
            }
            catch { }
        ...
        public GameClient GetClient()
        {
            ...
            return BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(Id);
        }
        ...
        {
            this.LastNameChange = BiosEmuThiago.GetUnixTimestamp();
        ...
        public void PrepareRoom(int Id, string Password)
        {
            ...
            {
				if (!BiosEmuThiago.GetGame().GetRoomManager().TryGetRoom(GetClient().GetHabbo().CurrentRoomId, out Room OldRoom))
					return;
            ...
            Room Room = BiosEmuThiago.GetGame().GetRoomManager().LoadRoom(Id);
            if (Room == null)
            {
                GetClient().SendMessage(new CloseConnectionComposer());
                return;
        ...
        public bool EnterRoom(Room Room)
        {
            ...
            using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.runFastQuery("INSERT INTO user_roomvisits (user_id,room_id,entry_timestamp,exit_timestamp,hour,minute) VALUES ('" + GetClient().GetHabbo().Id + "','" + GetClient().GetHabbo().CurrentRoomId + "','" + BiosEmuThiago.GetUnixTimestamp() + "','0','" + DateTime.Now.Hour + "','" + DateTime.Now.Minute + "');");// +
            }
            ...
            if (Room.OwnerId != Id)
            {
                ...
                BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(GetClient(), "ACH_RoomEntry", 1);
            }
            return true;
        }
    }

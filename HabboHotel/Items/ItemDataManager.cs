...
namespace Bios.HabboHotel.Items
{
    public class ItemDataManager
    {
        private static readonly ILog log = LogManager.GetLogger("Bios.HabboHotel.Items.ItemDataManager");
        ...
        public void Init()
        {
                ...
                if (ItemData != null)
                {
                    foreach (DataRow Row in ItemData.Rows)
                    {
                        try
                        {
                            ...
                            bool allowStack = BiosEmuThiago.EnumToBool(Row["can_stack"].ToString());
                            bool allowWalk = BiosEmuThiago.EnumToBool(Row["is_walkable"].ToString());
                            bool allowSit = BiosEmuThiago.EnumToBool(Row["can_sit"].ToString());
                            bool allowRecycle = BiosEmuThiago.EnumToBool(Row["allow_recycle"].ToString());
                            bool allowTrade = BiosEmuThiago.EnumToBool(Row["allow_trade"].ToString());
                            bool allowMarketplace = Convert.ToInt32(Row["allow_marketplace_sell"]) == 1;
                            bool allowGift = Convert.ToInt32(Row["allow_gift"]) == 1;
                            bool allowInventoryStack = BiosEmuThiago.EnumToBool(Row["allow_inventory_stack"].ToString());
                            ...
                            bool IsRare = BiosEmuThiago.EnumToBool(Row["is_rare"].ToString());
                            int ClothingId = Convert.ToInt32(Row["clothing_id"]);
                            bool ExtraRot = BiosEmuThiago.EnumToBool(Row["extra_rot"].ToString());
            ...
            log.Info("» Administrador de Items -> PRONTO - BY: Thiago Araujo");
        }

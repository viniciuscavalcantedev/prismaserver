...
using Bios.Communication.Packets.Outgoing.Rooms.Furni.LoveLocks;
            ...
            if (Gamemap.TilesTouching(Item.GetX, Item.GetY, User.X, User.Y))
            {
                if (Item.ExtraData == null || Item.ExtraData.Length <= 1 || !Item.ExtraData.Contains(Convert.ToChar(5).ToString()))
                {
                    ...
                    if(UserOne == null || UserTwo == null)
                        Session.SendNotification("Não conseguimos encontrar um usuário válido para bloquear esse bloco de amor.");
                    else if(UserOne.GetClient() == null || UserTwo.GetClient() == null)
                        Session.SendNotification("Não conseguimos encontrar um usuário válido para bloquear esse bloco de amor.");
                    else if(UserOne.HabboId != Item.UserID && UserTwo.HabboId != Item.UserID)
                        Session.SendNotification("Você só pode usar este item com o proprietário do item.");
                    else
                    {

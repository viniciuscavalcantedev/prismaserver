...
namespace Bios.HabboHotel.Items.Data.Moodlight
{
    public class MoodlightData
    {
        ...
        public MoodlightData(int ItemId)
        {
            ...
            if (Row == null)
            {
                using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.runFastQuery("INSERT INTO `room_items_moodlight` (item_id,enabled,current_preset,preset_one,preset_two,preset_three) VALUES (" + ItemId + ",0,1,'#000000,255,0','#000000,255,0','#000000,255,0')");
                    dbClient.SetQuery("SELECT enabled,current_preset,preset_one,preset_two,preset_three FROM room_items_moodlight WHERE item_id=" + ItemId + " LIMIT 1");
                    Row = dbClient.getRow();
                }
            }

            Enabled = BiosEmuThiago.EnumToBool(Row["enabled"].ToString());
        ...
        public static MoodlightPreset GeneratePreset(string Data)
        {
            ...
            return new MoodlightPreset(Bits[0], int.Parse(Bits[1]), BiosEmuThiago.EnumToBool(Bits[2]));
        }

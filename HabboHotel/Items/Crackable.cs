...
namespace Bios.HabboHotel.Items
{
    ...
    internal class CrackableManager
    {
        ...
        internal void ReceiveCrackableReward(RoomUser user, Room room, Item item)
        {
            ...
            if (item.GetBaseItem().InteractionType != InteractionType.PINATA && item.GetBaseItem().InteractionType != InteractionType.PINATATRIGGERED && item.GetBaseItem().InteractionType != InteractionType.RPGTHIAGO && item.GetBaseItem().InteractionType != InteractionType.MAGICEGG && item.GetBaseItem().InteractionType != InteractionType.MAGICCHEST) return;
                        ...
                        case "ptosfame":
                            {
                                ...
                                user.GetClient().SendMessage(RoomNotificationComposer.SendBubble("moedas", "Você acabou de ganhar " + int.Parse(reward.CrackableReward) + " "+ExtraSettings.PTOS_COINS+".", ""));
                                user.GetClient().SendMessage(RoomNotificationComposer.SendBubble("energy_low_warning", "Sua sorte foi de " + rand + " no dado. Parabéns!", ""));
                                room.GetRoomItemHandler().RemoveFurniture(user.GetClient(), item.Id);
                                dbClient.runFastQuery("DELETE FROM items WHERE id = " + item.Id);
                                return;

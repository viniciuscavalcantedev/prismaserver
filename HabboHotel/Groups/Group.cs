...
namespace Bios.HabboHotel.Groups
{
    public class Group
    {
        ...
        public Group(int Id, string Name, string Description, string Badge, int RoomId, int Owner, int Time, int Type, int Colour1, int Colour2, int AdminOnlyDeco, bool forumEnabled, bool hChat)
        {
            this.Id = Id;
            this.Name = Name;
            ...
            using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetRequests != null)
                {
                    foreach (DataRow Row in GetRequests.Rows)
                    {
                        ...
                        {
                            dbClient.runFastQuery("DELETE FROM `group_requests` WHERE `group_id` = '" + this.Id + "' AND `user_id` = '" + UserId + "'");
                        }

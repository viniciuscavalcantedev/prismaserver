...
namespace Bios.HabboHotel.Moderation
{
    ...
    {
        private static ILog log = LogManager.GetLogger("Bios.HabboHotel.Moderation.ModerationManager");
        ...
        public void Init()
        {
            ...
            using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetBans != null)
                {
                    foreach (DataRow dRow in GetBans.Rows)
                    {
                        ...
                        if (Ban != null)
                        {
                            if (expires > BiosEmuThiago.GetUnixTimestamp())
                            {
            ...
            log.Info("» PRONTO - BY: Thiago Araujo: " + (this._userPresets.Count + this._roomPresets.Count) + " presets de moderação.");
            log.Info("» PRONTO - BY: Thiago Araujo: " + this._userActionPresetCategories.Count + " categorias de moderação.");
            log.Info("» PRONTO - BY: Thiago Araujo: " + this._userActionPresetMessages.Count + " Msj. de pré-ajuste de ação de moderação.");
            log.Info("» Em cache: " + this._bans.Count + " usuários e máquinas de baneo! - BY: Thiago Araujo");
        }
        ...
        public void BanUser(string Mod, ModerationBanType Type, string BanValue, string Reason, double ExpireTimestamp)
        {
            ...
            using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("REPLACE INTO `bans` (`bantype`, `value`, `reason`, `expire`, `added_by`,`added_date`) VALUES ('" + BanType + "', '" + BanValue + "', @reason, " + ExpireTimestamp + ", '" + Mod + "', '" + BiosEmuThiago.GetUnixTimestamp() + "');");
        ...
        public bool MachineBanCheck(string MachineId)
        {
            ...
            if (BiosEmuThiago.GetGame().GetModerationManager().IsBanned(MachineId, out MachineBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        BiosEmuThiago.GetGame().GetModerationManager().RemoveBan(MachineId);
                        return false;
                    }
                }
            }

        ...
        public static void Search(ServerPacket Message, SearchResultList SearchResult, string SearchData, GameClient Session, int FetchLimit)
        {
            ...
            switch (SearchResult.CategoryType)
            {
                ...
                case NavigatorCategoryType.QUERY:
                    {
                        ...
                        if (SearchData.ToLower().StartsWith("owner:"))
                        {
                            if (SearchData.Length > 0)
                            {
                                ...
                                if (GetRooms != null)
                                {
                                    ...
                                    {
                                        RoomData RoomData = BiosEmuThiago.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row);
                        ...
                        else if (SearchData.ToLower().StartsWith("tag:"))
                        {
                            ...
                            ICollection<RoomData> TagMatches = BiosEmuThiago.GetGame().GetRoomManager().SearchTaggedRooms(SearchData);
                        ...
                        else if (SearchData.ToLower().StartsWith("group:"))
                        {
                            ...
                            ICollection<RoomData> GroupRooms = BiosEmuThiago.GetGame().GetRoomManager().SearchGroupRooms(SearchData);
                                ...
                                if (Table != null)
                                {
                                    foreach (DataRow Row in Table.Rows)
                                    {
                                        if (Convert.ToString(Row["state"]) == "invisible")
                                            continue;
                                        RoomData RData = BiosEmuThiago.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row);
                ...
                case NavigatorCategoryType.FEATURED:
                    {
                        ...
                        ICollection<FeaturedRoom> Featured = BiosEmuThiago.GetGame().GetNavigator().GetFeaturedRooms(SearchResult.Id);
                        foreach (FeaturedRoom FeaturedItem in Featured.ToList())
                        {
                            if (FeaturedItem == null)
                                continue;
                            RoomData Data = BiosEmuThiago.GetGame().GetRoomManager().GenerateRoomData(FeaturedItem.RoomId);
                            if (Data == null)
                                continue;
                ...
                case NavigatorCategoryType.STAFF_PICKS:
                    {
                        #region Featured
                        List<RoomData> rooms = new List<RoomData>();
                        ICollection<StaffPick> picks = BiosEmuThiago.GetGame().GetNavigator().GetStaffPicks();
                        foreach (StaffPick pick in picks.ToList())
                        {
                            if (pick == null)
                                continue;
                            RoomData Data = BiosEmuThiago.GetGame().GetRoomManager().GenerateRoomData(pick.RoomId);
                            if (Data == null)
                                continue;
                            if (!rooms.Contains(Data))
                                rooms.Add(Data);
                ...
                case NavigatorCategoryType.POPULAR:
                    {
                        List<RoomData> PopularRooms = BiosEmuThiago.GetGame().GetRoomManager().GetPopularRooms(-1, FetchLimit);
                ...
                case NavigatorCategoryType.RECOMMENDED:
                    {
                        List<RoomData> RecommendedRooms = BiosEmuThiago.GetGame().GetRoomManager().GetRecommendedRooms(FetchLimit);
                ...
                case NavigatorCategoryType.CATEGORY:
                    {
                        List<RoomData> GetRoomsByCategory = BiosEmuThiago.GetGame().GetRoomManager().GetRoomsByCategory(SearchResult.Id, FetchLimit);
                ...
                case NavigatorCategoryType.MY_FAVORITES:
                    ...
                    {
                        RoomData Room = BiosEmuThiago.GetGame().GetRoomManager().GenerateRoomData(Id);
                        if (Room == null)
                            continue;
                ...
                case NavigatorCategoryType.MY_GROUPS:
                    ...
                    foreach (Group Group in BiosEmuThiago.GetGame().GetGroupManager().GetGroupsForUser(Session.GetHabbo().Id).ToList())
                    {
                        if (Group == null)
                            continue;
                        RoomData Data = BiosEmuThiago.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId);
                        if (Data == null)
                            continue;
                ...
                case NavigatorCategoryType.MY_RIGHTS:
                    ...
                    using (IQueryAdapter dbClient = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
                    {
                        ...
                        {
                            RoomData Data = BiosEmuThiago.GetGame().GetRoomManager().GenerateRoomData(Convert.ToInt32(Row["room_id"]));
                            if (Data == null)
                                continue;
                ...
                case NavigatorCategoryType.TOP_PROMOTIONS:
                    {
                        List<RoomData> GetPopularPromotions = BiosEmuThiago.GetGame().GetRoomManager().GetOnGoingRoomPromotions(16, FetchLimit);
                ...
                case NavigatorCategoryType.PROMOTION_CATEGORY:
                    {
                        List<RoomData> GetPromotedRooms = BiosEmuThiago.GetGame().GetRoomManager().GetPromotedRooms(SearchResult.Id, FetchLimit);

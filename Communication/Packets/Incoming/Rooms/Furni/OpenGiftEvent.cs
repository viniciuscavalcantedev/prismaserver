...
namespace Bios.Communication.Packets.Incoming.Rooms.Furni
{
	class OpenGiftEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            UserCache Purchaser = BiosEmuThiago.GetGame().GetCacheManager().GenerateUser(PurchaserId);
            if (Purchaser == null)
            {
                Session.SendNotification("Ops! aparentemente há um erro com este presente.\nVamos livrar-se deste presente.");
                Room.GetRoomItemHandler().RemoveFurniture(null, Present.Id, false);
            ...
            if (!BiosEmuThiago.GetGame().GetItemManager().GetItem(Convert.ToInt32(Data["base_id"]), out BaseItem))
            {

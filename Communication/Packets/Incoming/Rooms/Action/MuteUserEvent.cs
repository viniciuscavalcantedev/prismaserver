...
namespace Bios.Communication.Packets.Incoming.Rooms.Action
{
    class MuteUserEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            {
                if (Room.MutedUsers[UserId] < BiosEmuThiago.GetUnixTimestamp())
                    Room.MutedUsers.Remove(UserId);
                else
                    return;
            }

            Room.MutedUsers.Add(UserId, (BiosEmuThiago.GetUnixTimestamp() + (Time * 60)));
          
            Target.GetClient().SendWhisper("O proprietário da sala silenciou por " + Time + " minutos!");
            BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_SelfModMuteSeen", 1);
        }
    }
}

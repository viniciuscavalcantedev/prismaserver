...
namespace Bios.Communication.Packets.Incoming.Rooms.Engine
{
    class PlaceObjectEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Room.GetRoomItemHandler().GetWallAndFloor.Count() > Convert.ToInt32(BiosEmuThiago.GetGame().GetSettingsManager().TryGetValue("room.item.placement_limit")))
            {
                Session.SendNotification("Você não pode ter mais de " + Convert.ToInt32(BiosEmuThiago.GetGame().GetSettingsManager().TryGetValue("room.item.placement_limit")) + " mobs em uma sala!");
                return;
            }
            ...
            if (!Item.IsWallItem)
            {
                ...
                if (Room.GetRoomItemHandler().SetFloorItem(Session, RoomItem, X, Y, Rotation, true, false, true))
                {
                    ...
                    if (Session.GetHabbo().Id == Room.OwnerId)
                        BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_RoomDecoFurniCount", 1, false);

...
namespace Bios.Communication.Packets.Incoming.Rooms.FloorPlan
{
	class SaveFloorPlanModelEvent : IPacketEvent
	{
		public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
		{
			...
			{
				Session.SendMessage(new RoomNotificationComposer("floorplan_editor.error", "errors", "Ops, parece que você digitou um floor inválido!"));
				return;
			}
			...
			if (!isValid)
			{
				Session.SendMessage(new RoomNotificationComposer("floorplan_editor.error", "errors", "Ops, parece que você entrou em quarto com o piso inválido!"));
				return;
			}
			...
			BiosEmuThiago.GetGame().GetRoomManager().ReloadModel(ModelName);

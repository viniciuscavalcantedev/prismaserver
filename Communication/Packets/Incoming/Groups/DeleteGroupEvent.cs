...
namespace Bios.Communication.Packets.Incoming.Groups
{
    class DeleteGroupEvent : IPacketEvent
    {
        ...
        {
            Group Group = null;
            if (!BiosEmuThiago.GetGame().GetGroupManager().TryGetGroup(Packet.PopInt(), out Group))
            {
            ...
            if (Group.MemberCount >= Convert.ToInt32(BiosEmuThiago.GetGame().GetSettingsManager().TryGetValue("group.delete.member.limit")) && !Session.GetHabbo().GetPermissions().HasRight("group_delete_limit_override"))
            {
                Session.SendNotification("Ops seu grupo já superrous a quantidade maxima  de mebros! (" + Convert.ToInt32(BiosEmuThiago.GetGame().GetSettingsManager().TryGetValue("group.delete.member.limit")) + ") que un grupo puede exceder antes de ser elegible para su eliminación. Solicitar asistencia de un miembro del staff.");
                return;
            }

            Room Room = BiosEmuThiago.GetGame().GetRoomManager().LoadRoom(Group.RoomId);
            ...
            BiosEmuThiago.GetGame().GetGroupManager().DeleteGroup(Group.Id);
            ...
            if (forumEnabled)
            {
                BiosEmuThiago.GetGame().GetGroupForumManager().RemoveGroup(Group);
                return;
            }

            //Unload it last.
            BiosEmuThiago.GetGame().GetRoomManager().UnloadRoom(Room.Id);

...
namespace Bios.Communication.Packets.Incoming.Groups
{
    class GetForumsListDataEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            {
                case 2:
                    var Forums = BiosEmuThiago.GetGame().GetGroupForumManager().GetForumsByUserId(Session.GetHabbo().Id);
                ...
                case 0:
                    ...
                    using (var adap = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
                    {
                        ...
                        adap.AddParameter("now", (int)BiosEmuThiago.GetUnixTimestamp());
                    ...
                    {
                        GroupForum forum;
                        if (BiosEmuThiago.GetGame().GetGroupForumManager().TryGetForum(Convert.ToInt32(Row["id"]), out forum))
                            forums.Add(forum);
                    }
                    break;

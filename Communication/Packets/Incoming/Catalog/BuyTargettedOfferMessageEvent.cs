...
namespace Bios.Communication.Packets.Incoming.Catalog
{
    class BuyTargettedOfferMessage : IPacketEvent
    {
...

        {
            ...
            var offer = BiosEmuThiago.GetGame().GetTargetedOffersManager().TargetedOffer;
                        ...
                        foreach (var product in offer.Products)
                        {
                            ...
                            switch (product.ItemType)
                            {
                                ...
                                case "item":
                                    {
                                        ItemData item = null;
                                        if (!BiosEmuThiago.GetGame().GetItemManager().GetItem(int.Parse(product.Item), out item)) return;
                                        if (item == null) return;
            ...
            TargetedOffers TargetedOffer2 = BiosEmuThiago.GetGame().GetTargetedOffersManager().TargetedOffer;
            using (IQueryAdapter dbQuery2 = BiosEmuThiago.GetDatabaseManager().GetQueryReactor())
            {
                ...
                foreach (DataRow Row2 in count2.Rows)
                {
                    ...
                    {
                        Session.SendMessage(BiosEmuThiago.GetGame().GetTargetedOffersManager().TargetedOffer.Serialize());
                    }
                }
            }
            #endregion

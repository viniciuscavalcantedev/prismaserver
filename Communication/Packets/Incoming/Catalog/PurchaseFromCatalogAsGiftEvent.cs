...
namespace Bios.Communication.Packets.Incoming.Catalog
{
    public class PurchaseFromCatalogAsGiftEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            if (BiosEmuThiago.GetGame().GetSettingsManager().TryGetValue("room.item.gifts.enabled") != "1")
            {
            ...
            if (!BiosEmuThiago.GetGame().GetItemManager().GetGift(SpriteId, out PresentData) || PresentData.InteractionType != InteractionType.GIFT)
                return;
            ...
            Habbo Habbo = BiosEmuThiago.GetHabboByUsername(GiftUser);
            if (Habbo == null)
            {
                    ...
                    case InteractionType.LANDSCAPE:
                        ...
                        try
                        {
                            if (string.IsNullOrEmpty(Data))
                                ...
                                Number = Double.Parse(Data, BiosEmuThiago.CultureInfo);
                        }
                        catch
                        {
            ...
            if (GiveItem != null)
            {
                GameClient Receiver = BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(Habbo.Id);
                if (Receiver != null)
                {
                ...
                {
                    BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_GiftGiver", 1);
                    if (Receiver != null)
                        BiosEmuThiago.GetGame().GetAchievementManager().ProgressAchievement(Receiver, "ACH_GiftReceiver", 1);
                    BiosEmuThiago.GetGame().GetQuestManager().ProgressUserQuest(Session, QuestType.GIFT_OTHERS);
                }
            }

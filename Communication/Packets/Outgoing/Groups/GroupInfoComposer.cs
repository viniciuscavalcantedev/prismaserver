...
namespace Bios.Communication.Packets.Outgoing.Groups
{
	class GroupInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.GroupInfoMessageComposer)
        {
			...
			WriteString((BiosEmuThiago.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId) == null) ? "Nenhum quarto encontrado ..." : BiosEmuThiago.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId).Name);    // room name
			...
			WriteString(BiosEmuThiago.GetUsernameById(Group.CreatorId));

...
namespace Bios.Communication.Packets.Outgoing.Rooms.AI.Pets
{
    class PetInformationComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.PetInformationMessageComposer)
        {
            ...
            if (!BiosEmuThiago.GetGame().GetRoomManager().TryGetRoom(Pet.RoomId, out Room))
                return;
            ...
            : base(ServerPacketHeader.PetInformationMessageComposer)
        {
            ...
            WriteInteger(Habbo.Id);
            WriteInteger(Convert.ToInt32(Math.Floor((BiosEmuThiago.GetUnixTimestamp() - Habbo.AccountCreated) / 86400)));//How?
            WriteString(Habbo.Username);
            WriteInteger(1);//3 on hab
            WriteBoolean(false);
            WriteBoolean(false);

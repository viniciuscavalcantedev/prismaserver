...
namespace Bios.Communication.Packets.Outgoing.Moderation
{
    class ModeratorUserInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.ModeratorUserInfoMessageComposer)
        {
			...
			WriteString(User != null ? Convert.ToString(User["username"]) : "Desconhecido");
			WriteString(User != null ? Convert.ToString(User["look"]) : "Desconhecido");
			WriteInteger(User != null ? Convert.ToInt32(Math.Ceiling((BiosEmuThiago.GetUnixTimestamp() - Convert.ToDouble(User["account_created"])) / 60)) : 0);
			WriteInteger(User != null ? Convert.ToInt32(Math.Ceiling((BiosEmuThiago.GetUnixTimestamp() - Convert.ToDouble(User["last_online"])) / 60)) : 0);
			WriteBoolean(User != null ? BiosEmuThiago.GetGame().GetClientManager().GetClientByUserID(Convert.ToInt32(User["id"])) != null : false);
			...
			WriteString(User != null ? Convert.ToString(User["mail"]) : "Desconhecido");
